...
export default function ArticleCategoriesIndex({ categories, filters = {} }) {
    ...
    function applyFilters(patch) {
        router.get('/admin/article-categories', { ...filters, ...patch }, { preserveState: true, replace: true });
    }
    ...
    function submit(event) {
        ...
        if (editingId) {
            put(`/admin/article-categories/${editingId}`, options);
            return;
        }

        post('/admin/article-categories', options);
    }

    function remove(category) {
        if (!confirm(`Hapus kategori "${category.name}"? Artikel terkait akan jadi tanpa kategori.`)) return;
        router.delete(`/admin/article-categories/${category.id}`, { preserveScroll: true });
    }
            ...
            <div className="grid gap-6 lg:grid-cols-[420px_1fr]">
                <form onSubmit={submit} className="space-y-4 rounded-lg border bg-white p-4 shadow-sm">
                    <div>
                        ...
                        <p className="text-sm text-slate-500">Kategori dipakai di filter blog dan pengelompokan artikel.</p>
                    </div>
                            ...
                            <tbody className="divide-y divide-slate-200">
                                {categories.data.map((category) => (
                                    <tr key={category.id}>
                                        ...
                                        <td className="px-4 py-3 text-center text-slate-600">{category.articles_count}</td>
                                        <td className="px-4 py-3">
                                ...
                                {categories.data.length === 0 && (
                                    <tr>
                                        <td colSpan="4" className="px-4 py-10 text-center text-sm text-slate-500">Belum ada kategori artikel.</td>
                                    </tr>
                                )}
                            </tbody>
                        </table>
